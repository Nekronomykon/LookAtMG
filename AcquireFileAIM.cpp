  ...
  do
  {
    ...
    else if (!skip.find("BCP"))
    {
      ...
      if (type == cpTypeSaddleB)
        idElementAdd = 0; // --> change to the AddSpecialAtom(* * *) call
    ...
    else if (!skip.find("RCP"))
    {
      ...
      if (type == cpTypeSaddleR)
        idElementAdd = Elements::idHe; // fictituous He --> change to the AddSpecialAtom(* * *) call
      else
      {
    ...
    else if (!skip.find("CCP"))
    {
      ...
      {
        idElementAdd = Elements::idNe; // fictituous Ne: local minimum --> change to the AddSpecialAtom(* * *) call
      }
      else
      {
        vtkErrorMacro("AcquireQTAIMFile error: CP record #" << ++nReadCP

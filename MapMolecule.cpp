...
vtkObjectFactoryNewMacro(MapOfMolecule);

//------------------------------------------------------------------------------
MapOfMolecule::MapOfMolecule()
    : AtomicRadiusType(VDWRadius),
      AtomicRadiusScaleFactor(0.3), AtomicRadiusArrayName(nullptr), AtomColorMode(DiscreteByAtom),
      RenderBonds(true), BondColorMode(DiscreteByAtom), UseMultiCylindersForBonds(true), BondRadius(0.075),
      RenderLattice(true)
{
    // Initialize ivars:
    ...
    sphere->Update();
    this->mapAtom_->SetSourceConnection(sphere->GetOutputPort());
    ...
    this->mapBond_->SetSourceConnection(cylXformFilter->GetOutputPort());
    ...
    // Setup glyph mappers
    this->mapAtom_->SetScalarRange(0, this->PeriodicTable->GetNumberOfElements());
    this->mapAtom_->SetColorModeToMapScalars();
    this->mapAtom_->SetScalarModeToUsePointFieldData();
    this->mapAtom_->SetScaleModeToScaleByMagnitude();
    this->mapBond_->SetScaleModeToScaleByVectorComponents();
    ...
    this->mapAtom_->AddObserver(vtkCommand::StartEvent, cb);
    this->mapAtom_->AddObserver(vtkCommand::EndEvent, cb);
    this->mapAtom_->AddObserver(vtkCommand::ProgressEvent, cb);

    this->mapBond_->AddObserver(vtkCommand::StartEvent, cb);
    this->mapBond_->AddObserver(vtkCommand::EndEvent, cb);
    this->mapBond_->AddObserver(vtkCommand::ProgressEvent, cb);

    // Connect the trivial producers to forward the glyph polydata
    this->outputAtom_->SetOutput(this->dataAtom_);
    this->mapAtom_->SetInputConnection(this->outputAtom_->GetOutputPort());

    this->outputBond_->SetOutput(this->dataBond_);
    this->mapBond_->SetInputConnection(this->outputBond_->GetOutputPort());
...
//------------------------------------------------------------------------------
void MapOfMolecule::SetInputData(vtkMolecule *input)
{
    this->SetInputDataInternal(0, input);
}

//------------------------------------------------------------------------------
vtkMolecule *MapOfMolecule::GetInput()
{
    return vtkMolecule::SafeDownCast(this->GetExecutive()->GetInputData(0, 0));
}

//------------------------------------------------------------------------------
void MapOfMolecule::UseBallAndStickSettings()
{
...
//------------------------------------------------------------------------------
void MapOfMolecule::UseVDWSpheresSettings()
{
    this->SetRenderBonds(false); // behavior is changed
    this->SetAtomicRadiusType(VDWRadius);
...
//------------------------------------------------------------------------------
void MapOfMolecule::UseLiquoriceStickSettings()
{
    this->SetRenderBonds(true);
...
//------------------------------------------------------------------------------
void MapOfMolecule::UseFastSettings()
{
    this->SetRenderBonds(true);
    this->SetAtomicRadiusType(UnitRadius);
...
//------------------------------------------------------------------------------
const char *MapOfMolecule::GetAtomicRadiusTypeAsString()
{
...
//------------------------------------------------------------------------------
const char *MapOfMolecule::GetBondColorModeAsString()
{
...
//------------------------------------------------------------------------------
void MapOfMolecule::GetSelectedAtomsAndBonds(
...
//------------------------------------------------------------------------------
void MapOfMolecule::Render(vtkRenderer *ren, vtkActor *act)
{
...
//------------------------------------------------------------------------------
void MapOfMolecule::GlyphRender(vtkRenderer *ren, vtkActor *act)
{
    // Update cached polydata if needed
    this->UpdateGlyphPolyData();

    // Pass rendering call on
    this->mapAtom_->Render(ren, act);

    if (this->RenderBonds)
    {
        this->mapBond_->Render(ren, act);
    }
...
//------------------------------------------------------------------------------
void MapOfMolecule::UpdateGlyphPolyData()
{
    vtkMolecule *molecule = this->GetInput();

    if (!this->GlyphDataInitialized ||
        ((molecule->GetMTime() > this->dataAtom_->GetMTime() ||
          this->GetMTime() > this->dataAtom_->GetMTime() ||
          this->LookupTable->GetMTime() > this->dataAtom_->GetMTime())))
    {
        this->UpdateAtomGlyphPolyData();
    }

    if (!this->GlyphDataInitialized ||
        ((molecule->GetMTime() > this->dataBond_->GetMTime() ||
          this->GetMTime() > this->dataBond_->GetMTime() ||
          this->LookupTable->GetMTime() > this->dataBond_->GetMTime()) &&
         this->RenderBonds))
    {
...
void MapOfMolecule::UpdateAtomGlyphPolyData()
{
    this->dataAtom_->Initialize();
    ...
    case SingleColor:
    {
        ...
        this->dataAtom_->GetPointData()->SetScalars(singleColorArray);
        break;
    }
    case DiscreteByAtom:
    default:
        ...
        this->mapAtom_->SetLookupTable(this->LookupTable);
        break;
    }
    ...
    if (colorArray != nullptr)
    {
        ...
        {
            int colorArrayIdx = this->dataAtom_->GetPointData()->AddArray(colorArray);
            this->mapAtom_->SelectColorArray(colorArrayIdx);
        }
        colorArray->Delete();
    }

    this->dataAtom_->SetPoints(points.Get());
    ...
    this->dataAtom_->GetPointData()->AddArray(scaleFactors);
    this->mapAtom_->SetScaleArray("Scale Factors");
}

//------------------------------------------------------------------------------
// Generate position, scale, and orientation vectors for each bond cylinder
void MapOfMolecule::UpdateBondGlyphPolyData()
{
    this->dataBond_->Initialize();
    ...
    // mappers of MapOfMolecule, use a custom selection ID range. This also
    ...
    this->dataBond_->SetPoints(cylCenters);
    this->dataBond_->GetPointData()->AddArray(cylScales);
    this->dataBond_->GetPointData()->AddArray(orientationVectors);
    this->dataBond_->GetPointData()->AddArray(selectionIds);
    ...
    if (useColorArray)
    {
        ...
        this->mapBond_->SetScalarRange(0, this->PeriodicTable->GetNumberOfElements());
        this->mapBond_->SetColorMode(this->mapAtom_->GetColorMode());
        this->mapBond_->SetScalarModeToUsePointFieldData();
        this->mapBond_->SetLookupTable(this->LookupTable);
    }
    ...
    else
    {
        ...
        this->dataBond_->GetPointData()->SetScalars(singleColorArray);
        this->mapBond_->SetColorModeToDefault();
        this->mapBond_->SetScalarModeToUsePointData();
    }
    ...
    if (useColorArray)
    {
        int colorArrayIdx = this->dataBond_->GetPointData()->AddArray(cylColors);
        this->mapBond_->SelectColorArray(colorArrayIdx);
    }
    cylColors->Delete();
    // Free up some space
    this->dataBond_->Squeeze();

    // Setup glypher
    this->mapBond_->SetScaleArray("Scale Factors");
    this->mapBond_->SetOrientationArray("Orientation Vectors");
    this->mapBond_->SetSelectionIdArray("Selection Ids");
    this->mapBond_->UseSelectionIdsOn();
}

//------------------------------------------------------------------------------
void MapOfMolecule::UpdateLatticePolyData()
{
...
//------------------------------------------------------------------------------
void MapOfMolecule::ReleaseGraphicsResources(vtkWindow *w)
{
    this->mapAtom_->ReleaseGraphicsResources(w);
    this->mapBond_->ReleaseGraphicsResources(w);
}

double *MapOfMolecule::GetBounds()
{
...
//------------------------------------------------------------------------------
int MapOfMolecule::FillInputPortInformation(int vtkNotUsed(port), vtkInformation *info)
{
    info->Set(vtkAlgorithm::INPUT_REQUIRED_DATA_TYPE(), "vtkMolecule"); // to think, when forking the molecule
    return 1;
}

//------------------------------------------------------------------------------
void MapOfMolecule::PrintSelf(ostream &os, vtkIndent indent)
{
    this->Superclass::PrintSelf(os, indent);

    os << indent << "AtomGlyphMapper:\n";
    this->mapAtom_->PrintSelf(os, indent.GetNextIndent());

    os << indent << "BondGlyphMapper:\n";
    this->mapBond_->PrintSelf(os, indent.GetNextIndent());
}

//------------------------------------------------------------------------------
void MapOfMolecule::SetMapScalars(bool map)
{
    this->mapAtom_->SetColorMode(
        map ? VTK_COLOR_MODE_MAP_SCALARS : VTK_COLOR_MODE_DIRECT_SCALARS);
    this->mapBond_->SetColorMode(
        map ? VTK_COLOR_MODE_MAP_SCALARS : VTK_COLOR_MODE_DIRECT_SCALARS);
}
